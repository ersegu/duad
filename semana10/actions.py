    ...
    try:
        while True:
            ...
            if student is None:
                ...
                student = {'name': name.title(), 'lastname': lastname.title(), 'section':section.upper(), 'spanish':spanish_grade,'english':english_grade,'social':social_grade,'science':science_grade, 'average':average}
...
def print_student_information(student_list):
    try:
        for student in student_list:
            print(student)
    except(IndexError, KeyError, AttributeError) as ex:
...
def print_top_3_students(student_list):
    try:
        ...
        for student in top_students:
            print(student)
    except (IndexError,ValueError,AttributeError, KeyError, TypeError) as ex:
...
def print_failed_students(student_list):
    ...
    try:
        for student in student_list:
            if float(student['spanish']) < 60 or float(student['english']) < 60 or float(student['social']) < 60 or float(student['science']) < 60:
                failed_student['name'] = student['name']
                failed_student['lastname'] = student['lastname']
                failed_student['section'] = student['section']
                if float(student['spanish']) < 60:
                    failed_student['spanish'] = student['spanish']
                if float(student['english']) < 60:
                    failed_student['english'] = student['english']
                if float(student['social']) < 60:
                    failed_student['social'] = student['social']
                if float(student['science']) < 60:
                    failed_student['science'] = student['science']
